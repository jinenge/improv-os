...
const ALLOW = new Set(['ai', 'http', 'store', 'compute']);
...
async function handle(cap, method, args, appId) {
  ...
  if (cap === 'compute' && method === 'run') {
    const r = await fetch('/api/capability/compute', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ task: String(args?.task || '').slice(0, 2000), appName: appId }),
    });
    const body = await r.json();
    if (!r.ok) throw new Error(body.error || '请求失败');
    return body.result;
