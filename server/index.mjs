...
6. 中文为主（明显的外文站点可用对应语言）。监听 Enter 的输入框（搜索框等）keydown 里先 if (e.isComposing || e.keyCode === 229) return; ——中文输入法选词的回车不触发提交。
...
function generateFast(req, res, { type, q, slug, lan }) {
...
const server = http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/generate') {
    ...
    return generateFast(req, res, { type, q, slug: type === 'search' ? slugify(q) : null, lan });
  }
